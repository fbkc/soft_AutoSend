    ...
    internal class HttphelperBase
    {
        ...
        private byte[] GetByte()
        {
            ...
            using (MemoryStream _stream = new MemoryStream())
            {
                ...
                {
                    //开始读取流并设置编码方式
                    InternalCopyTo(new GZipStream(response.GetResponseStream(), CompressionMode.Decompress),_stream, 10240);
                }
                else
                {
                    //开始读取流并设置编码方式                    
                    InternalCopyTo(response.GetResponseStream(),_stream, 10240);
                }
                //获取Byte
        ...
        #endregion
        private void InternalCopyTo(Stream a,Stream destination, int bufferSize)
        {
        ...
        private void SetRequest(HttpItem item)
        {
            ...
            {
                request.Headers.Add("Host", item.Host);                
            }
        ...
        private void SetCer(HttpItem item)
        {
            ...
            {
                //将证书添加到请求里
                if (!string.IsNullOrEmpty(item.CerPwd))
                {
